...
function PortfolioChart({ history, displayCurrency }: { history: ValueSnapshot[], displayCurrency: Currency }) {
  ...
  const width = 240, height = 60, padding = 10;
  const data = history.map(h => ({ date: h.date, value: displayCurrency === "EUR" ? h.valueEur : h.valueEur * EUR_TO_DKK }));
  const chartData = data.length === 1 ? [{ date: data[0].date - 86400000, value: data[0].value }, data[0]] : data;
    ...
    <div className="w-full lg:w-[320px] xl:w-[380px] bg-secondary/30 border rounded-xl p-3 flex items-center gap-4">
      ...
      <div className="flex-1 relative h-[45px]">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
          <defs>
            <linearGradient id="gradUp" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#22c55e" stopOpacity="0.3" /><stop offset="100%" stopColor="#22c55e" stopOpacity="0" /></linearGradient>
            <linearGradient id="gradDown" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ef4444" stopOpacity="0.3" /><stop offset="100%" stopColor="#ef4444" stopOpacity="0" /></linearGradient>
          </defs>
          ...
          <circle cx={padding + ((chartData[chartData.length - 1].date - minDate) / rangeDate) * (width - padding * 2)} cy={height - padding - ((chartData[chartData.length - 1].value - minVal) / rangeVal) * (height - padding * 2)} r="3.5" fill={strokeColor} className="animate-pulse" />
        </svg>
      </div>
    </div>
  );
...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    if (localData) {
      setPortfolios(JSON.parse(localData).map((p: any) => ({ ...p, history: p.history || [] })))
    }
    setLoading(false)
  ...
  const savePortfolios = (newPortfolios: Portfolio[]) => {
    ...
    fetch('/api/portfolios', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ portfolios: newPortfolios }) }).catch(() => {})
  }
  ...
  const runSearch = async (query: string) => {
    ...
    } catch (e) { toast.error("Failed to search cards.") } 
  ...
  const handleAddCard = (apiCard: any) => {
    ...
    const updated = portfolios.map(p => {
      ...
      return { ...p, cards: updatedCards, history: [...(p.history || []), { date: Date.now(), valueEur: newTotalEur }] };
    })
  ...
  const syncMarketPrices = async () => {
    ...
    try {
      ...
      const updatedCards = p.cards.map(card => {
        ...
        return freshPrice !== undefined ? { ...card, marketPrice: freshPrice } : card;
      });

      const newTotalEur = calculateTotalValue(updatedCards, "EUR");
      const updatedPortfolios = portfolios.map(port => 
        port.id === activePortfolioId ? { ...port, cards: updatedCards, history: [...(port.history || []), { date: Date.now(), valueEur: newTotalEur }] } : port
      );
    ...
    <div className="min-h-screen bg-background relative">
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div onClick={() => setDisplayCurrency(prev => prev === "EUR" ? "DKK" : "EUR")} className="flex items-center gap-2 cursor-pointer hover:bg-secondary/40 p-2 rounded-lg transition-colors border shadow-sm w-fit bg-card" title="Toggle Currency">
            <Coins className="h-4 w-4 text-[#4f5f4f]" />
            <span className="text-sm font-bold text-foreground">{displayCurrency === "EUR" ? "EUR (€)" : "DKK (kr)"}</span>
            ...
            {portfolios.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {portfolios.map(port => {
                  ...
                  return (
                    <div key={port.id} onClick={() => setActivePortfolioId(port.id)} className="group flex flex-col p-5 rounded-xl border bg-card hover:border-[#4f5f4f]/50 hover:shadow-md transition-all cursor-pointer relative">
                      <button onClick={(e) => { e.stopPropagation(); handleDeletePortfolio(port.id); }} className="absolute top-3 right-3 p-1.5 rounded-md bg-destructive/10 text-destructive opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive hover:text-white"><Trash2 className="h-3.5 w-3.5" /></button>
                      ...
                      <div className="mt-auto pt-4 border-t">
                        ...
                        <p className="text-xl font-bold text-[#4f5f4f]">{displayCurrency === "EUR" ? "€" : "kr "}{totalVal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                      </div>
                    </div>
                  )
                })}
        ...
        {activePortfolioId && activePortfolio && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <button onClick={() => setActivePortfolioId(null)} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"><ArrowLeft className="h-4 w-4" /> Back to Portfolios</button>
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b pb-6">
              ...
              <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row items-start sm:items-center lg:items-end xl:items-center gap-4 flex-shrink-0">
                <div className="text-left sm:text-right">
                  ...
                  <p className="text-2xl font-bold text-[#4f5f4f]">{displayCurrency === "EUR" ? "€" : "kr "}{calculateTotalValue(activePortfolio.cards, displayCurrency).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <Button onClick={syncMarketPrices} variant="outline" size="sm" className="gap-2 flex-1 sm:flex-none border-[#4f5f4f] text-[#4f5f4f] hover:bg-[#4f5f4f]/10" disabled={isSyncing}>
                    {isSyncing ? <Spinner className="h-3.5 w-3.5" /> : <RefreshCw className="h-3.5 w-3.5" />} Sync Market
                  </Button>
                  <Button size="sm" onClick={() => setIsAddingCard(true)} className="gap-2 flex-1 sm:flex-none bg-[#4f5f4f] hover:bg-[#4f5f4f]/90 text-white"><Plus className="h-4 w-4"/> Add Card</Button>
                </div>
              </div>
            </div>
                    ...
                    <div key={card.uniqueId} className="group relative flex flex-col">
                      <div className="relative aspect-[2.5/3.5] rounded-xl overflow-hidden shadow-sm border bg-secondary/20">
                        ...
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                          <Button variant="secondary" size="sm" className="gap-2 text-[#4f5f4f]" onClick={() => setEditingCard(card)}><Edit className="h-4 w-4" /> Edit Details</Button>
                        </div>
                        <div className="absolute bottom-2 left-2 z-10">
                          {card.acquiredMethod === "pulled" ? (
                            <span className="bg-red-500 text-white text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-sm border border-red-600">Ripped</span>
                          ) : (
                            <span className="bg-blue-600 text-white text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-sm border border-blue-700">Purchased</span>
                          )}
                        </div>
                      </div>
                      <div className="mt-3 flex flex-col px-1">
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          <p className="text-sm font-bold text-[#4f5f4f]">{displayCurrency === "EUR" ? "€" : "kr "}{cardVal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                          {card.paidPrice !== null && (<span className="text-[9px] text-muted-foreground bg-secondary px-1.5 rounded" title="Custom Value override active">Custom</span>)}
                        </div>
                      </div>
                    </div>
                  )
